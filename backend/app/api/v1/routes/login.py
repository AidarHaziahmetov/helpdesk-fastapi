...
from app.cruds.user import authenticate_async, get_user_by_email_async
...
async def recover_password(email: str, session: AsyncSessionDep) -> Message:
    ...
    Password Recovery
    """
    user = await get_user_by_email_async(session=session, email=email)
